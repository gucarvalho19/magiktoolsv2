...
interface CreateManualMembershipRequest {
  email: string;
  kiwifyOrderId: string;
  claimCode: string;
  purchasedAt?: string; // ISO date string, optional
  customerCpf?: string;
}

interface CreateManualMembershipResponse {
  success: boolean;
  membership: {
    id: number;
    email: string;
 ...
 */
export const createManualMembership = api<CreateManualMembershipRequest, CreateManualMembershipResponse>(
  ...
  async (req) => {
    ...
    try {
      // Check if membership already exists for this email or order
      const existing = await tx.queryRow<{ id: number }>`
      ...
      // Count active memberships to determine initial status
      ...
      await tx.exec`
        ...
        VALUES (
          ${auth.userID},
          'create_manual_membership',
          ${result.id},
          'Manual creation for purchase before webhook implementation',
          NOW()
        )
      `;
